    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <span className="text-[10px] font-mono text-zinc-500 w-5 text-right">#{rank}</span>
          {cr.image ? (
            <img src={cr.image} alt={cr.sym} className="w-7 h-7 rounded-full" onError={(e) => { e.target.style.display = 'none'; }} />
          ) : (
            <div className="w-7 h-7 rounded-full bg-surface flex items-center justify-center text-[10px] font-medium text-zinc-400">
              {cr.sym.charAt(0)}
            </div>
          )}
          <div>
            <div className="font-semibold text-white text-sm">{cr.sym}</div>
            <div className="text-[10px] text-zinc-500 truncate max-w-[80px]">{cr.name}</div>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between text-xs px-3 py-2.5 bg-surface border border-surface-border rounded-lg">
      ...
      {exp && (
        <div className="mt-4 pt-4 border-t border-card-border space-y-3 animate-fadeInUp">
          <div className="grid grid-cols-3 gap-2 text-center">
            {[{ l: '24H', v: cr.c24 }, { l: '7J', v: cr.c7 }, { l: '30J', v: cr.c30 }].map((x, i) => (
              <div key={i} className="bg-surface border border-surface-border rounded-lg p-2.5">
